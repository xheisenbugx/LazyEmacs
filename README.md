...
shortcuts can intercept Super keys; use the `SPC z` alternatives instead.
...
starting the server. `SPC c l` starts LSP, `SPC c R` restarts it, and `SPC c x`
shuts it down. Definitions use xref when LSP is not attached.
...
Formatting is explicit with `SPC c f`; `SPC u f` toggles format-on-save for the
...
`C-_`) toggles the current tab's terminal pane; `SPC p t` opens a project
terminal and `SPC p T` creates a fresh one. There is no fallback terminal setup.
...
| LSP does not start | Check `exec-path`, major mode, environment, and matching server; try `SPC c l` |
...
The following reference describes the integrated editing workflow. Mail keys
are available only after opting into mail. A standalone copy is also available
